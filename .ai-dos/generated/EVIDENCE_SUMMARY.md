...
| EVIDENCE:SPR25-002:PAYMENT-REQUEST-QR-CHECKOUT | SPR25-002 | TEST | Payment request, migration, and frontend checkout contracts | PASS | npm.cmd test -- --runInBand src/modules/payments/payment-request.service.spec.ts; npm.cmd run test:demo-data -- --runInBand prisma/payment-request-migration.spec.ts; npm.cmd test -- --run src/features/payments/PaymentCheckout.test.tsx src/features/commerce/CartPage.test.tsx src/features/memberships/MembershipPage.test.tsx |
| EVIDENCE:SPR25-002:PAYMENT-REQUEST-QR-CHECKOUT | SPR25-002 | TEST | Complete Backend, Frontend, HTTP, and persistence regression | PASS | npm.cmd test -- --runInBand; npm.cmd run test:e2e -- --runInBand; npm.cmd run test:demo-data -- --runInBand; npm.cmd test -- --run |
| EVIDENCE:SPR25-002:PAYMENT-REQUEST-QR-CHECKOUT | SPR25-002 | BUILD | Schema, production builds, and endpoint-security gates | PASS | npm.cmd run prisma:validate; npm.cmd run build; npm.cmd run security:audit-endpoints |
| EVIDENCE:SPR25-002:PAYMENT-REQUEST-QR-CHECKOUT | SPR25-002 | TEST | Responsive server-authoritative QR checkout | PASS | npx.cmd playwright test sprint24-membership-learner.spec.ts --project=chromium --no-deps --reporter=line |
| EVIDENCE:SPR25-002:PAYMENT-REQUEST-QR-CHECKOUT | SPR25-002 | DEPLOY | Exact-revision CI, production deployment, and readiness | PASS | public GitHub Actions run/job metadata and body-discarding HTTPS status checks |
| EVIDENCE:SPR25-002:PAYMENT-REQUEST-QR-CHECKOUT | SPR25-002 | OTHER | Security, dependency, provider activation, and diff review | PASS | npm.cmd audit --omit=dev --audit-level=high; git diff --check; staged credential-material scan; multi-axis review |
| EVIDENCE:SPR25-003:SIGNED-WEBHOOK-INGESTION | SPR25-003 | TEST | Official PayOS signature and authoritative settlement contracts | PASS | npm.cmd test -- --runInBand src/modules/payments/payment-webhook.service.spec.ts src/modules/payments/payment-webhook.controller.spec.ts src/modules/payments/payment-request.service.spec.ts src/modules/payments/payos-payment.provider.spec.ts src/common/audit/audit.service.spec.ts |
| EVIDENCE:SPR25-003:SIGNED-WEBHOOK-INGESTION | SPR25-003 | TEST | Complete Backend, Frontend, HTTP, and persistence regression | PASS | npm.cmd test -- --runInBand; npm.cmd run test:e2e -- --runInBand; npm.cmd run test:demo-data -- --runInBand; npm.cmd test -- --run |
| EVIDENCE:SPR25-003:SIGNED-WEBHOOK-INGESTION | SPR25-003 | BUILD | Additive schema, migration, build, and endpoint-security gates | PASS | npm.cmd run prisma:generate; npm.cmd run prisma:validate; npm.cmd run build; npm.cmd run security:audit-endpoints; migration contract and PostgreSQL suites |
| EVIDENCE:SPR25-003:SIGNED-WEBHOOK-INGESTION | SPR25-003 | DEPLOY | Exact-revision CI, production deployment, migrations, and readiness | PASS | authenticated GitHub Actions metadata; whitelisted job-log assertions; body-discarding HTTPS status checks |
| EVIDENCE:SPR25-003:SIGNED-WEBHOOK-INGESTION | SPR25-003 | OTHER | Provider-disable, privacy, dependency, and diff review | PASS | status-only production webhook checks; npm.cmd audit --omit=dev --audit-level=high; git diff --check; staged credential-material scan; final multi-axis review |
| EVIDENCE:SPR25-004:ATOMIC-ORDER-FULFILLMENT | SPR25-004 | TEST | Atomic fulfillment, retry, mixed-order, membership, and access contracts | PASS | node ./node_modules/jest/bin/jest.js --runInBand src/modules/payments/commerce-fulfillment.service.spec.ts src/modules/payments/payment-webhook.service.spec.ts src/modules/payments/payment-request.service.spec.ts src/modules/memberships/membership-checkout.service.spec.ts src/modules/access/course-access.service.spec.ts src/modules/access/course-access.rules.spec.ts |
| EVIDENCE:SPR25-004:ATOMIC-ORDER-FULFILLMENT | SPR25-004 | TEST | Complete Backend, HTTP, and persistence regression | PASS | npm.cmd test -- --runInBand; npm.cmd run test:e2e -- --runInBand; node --experimental-vm-modules ./node_modules/jest/bin/jest.js --runInBand --config prisma/jest.config.cjs |
| EVIDENCE:SPR25-004:ATOMIC-ORDER-FULFILLMENT | SPR25-004 | BUILD | Schema, migration, build, and endpoint-security gates | PASS | npm.cmd run prisma:generate; npm.cmd run prisma:validate; npm.cmd run build; npm.cmd run security:audit-endpoints; git diff --check |
| EVIDENCE:SPR25-004:ATOMIC-ORDER-FULFILLMENT | SPR25-004 | DEPLOY | Exact-revision CI, production deployment, and readiness | PASS | public GitHub Actions run/job metadata; checked-in deployment workflow inspection; body-discarding HTTPS status checks |
| EVIDENCE:SPR25-004:ATOMIC-ORDER-FULFILLMENT | SPR25-004 | OTHER | Provider isolation, privacy, dependency, and final diff review | PASS | status-only production webhook check; npm.cmd audit --omit=dev; staged secret-pattern review; git diff --check; final multi-axis review |
| EVIDENCE:SPR25-005:PAYMENT-RECONCILIATION-REVIEW | SPR25-005 | TEST | Missed callback, provider failure, checkpoint, and recovery contracts | PASS | npx jest --runInBand src/modules/payments/payment-reconciliation.service.spec.ts src/modules/payments/payment-reconciliation.controller.spec.ts src/modules/payments/payos-payment.provider.spec.ts src/modules/payments/payment-webhook.service.spec.ts |
| EVIDENCE:SPR25-005:PAYMENT-RECONCILIATION-REVIEW | SPR25-005 | TEST | Complete Backend, HTTP, persistence, and Frontend regression | PASS | npm.cmd test -- --runInBand; npm.cmd run test:e2e -- --runInBand; node --experimental-vm-modules ./node_modules/jest/bin/jest.js --runInBand --config prisma/jest.config.cjs; npm.cmd test -- --run |
| EVIDENCE:SPR25-005:PAYMENT-RECONCILIATION-REVIEW | SPR25-005 | BUILD | Schema, build, endpoint security, privacy, and dependency gates | PASS | npm.cmd run prisma:generate; npm.cmd run prisma:validate; npm.cmd run build; npm.cmd run security:audit-endpoints; git diff --check; npm.cmd audit --omit=dev |
| EVIDENCE:SPR25-005:PAYMENT-RECONCILIATION-REVIEW | SPR25-005 | DEPLOY | Exact-revision CI, production deployment, and route safety | PASS | public GitHub Actions run/job metadata; body-discarding production route checks |
| EVIDENCE:SPR25-005:PAYMENT-RECONCILIATION-REVIEW | SPR25-005 | OTHER | Operational review, provider isolation, and final diff review | PASS | runbook review; staged sensitive-pattern review; final multi-axis review |
| EVIDENCE:SPR25-006:PAYMENT-LIFECYCLE-REFUNDS | SPR25-006 | TEST | Cancellation, expiry, late-settlement, and concurrency contracts | PASS | npx jest --runInBand src/modules/payments/payment-cancellation.service.spec.ts src/modules/payments/payment-cancellation.controller.spec.ts src/modules/payments/payment-expiry.service.spec.ts src/modules/payments/payment-expiry.controller.spec.ts |
| EVIDENCE:SPR25-006:PAYMENT-LIFECYCLE-REFUNDS | SPR25-006 | TEST | Audited manual refund and access-consequence contracts | PASS | npx jest --runInBand src/modules/payments/payment-refund.service.spec.ts src/modules/payments/payment-refund.controller.spec.ts |
| EVIDENCE:SPR25-006:PAYMENT-LIFECYCLE-REFUNDS | SPR25-006 | TEST | Complete Backend, HTTP, persistence, and Frontend regression | PASS | npm.cmd test -- --runInBand; npm.cmd run test:e2e -- --runInBand; node --experimental-vm-modules ./node_modules/jest/bin/jest.js --runInBand --config prisma/jest.config.cjs; npm.cmd test -- --run |
| EVIDENCE:SPR25-006:PAYMENT-LIFECYCLE-REFUNDS | SPR25-006 | BUILD | Schema, build, endpoint-security, privacy, and dependency gates | PASS | npm.cmd run prisma:validate; npm.cmd run build; npm.cmd run security:audit-endpoints; git diff --check; npm.cmd audit --omit=dev --audit-level=high |
| EVIDENCE:SPR25-006:PAYMENT-LIFECYCLE-REFUNDS | SPR25-006 | DEPLOY | Exact-revision CI, production deployment, and protected-route safety | PASS | public GitHub Actions metadata; body-discarding production readiness and authorization checks |
| EVIDENCE:SPR25-006:PAYMENT-LIFECYCLE-REFUNDS | SPR25-006 | OTHER | Final security, access, history, and provider-isolation review | PASS | final diff review; sensitive-pattern review; lifecycle and access-consequence review |
| EVIDENCE:SPR25-007:CART-CLOSED-PAYMENT-VERIFICATION | SPR25-007 | PRODUCTION | Closed-payment Cart notice regression | PASS | Operator-reported production browser verification after Front-End 7ac6cda and Back-End ae81dc3 |
| EVIDENCE:SPR25-007:COURSE-PAYMENT-REQUEST-FAILURE | SPR25-007 | PRODUCTION | Bounded course UI journey reached the existing order | BLOCKED | Sanitized Playwright Course -> Cart -> Order journey on production |
| EVIDENCE:SPR25-007:COURSE-PAYMENT-REQUEST-FAILURE | SPR25-007 | PRODUCTION | Single course payment request failed before a payment attempt | BLOCKED | One bounded payment request on the existing owned course order |
| EVIDENCE:SPR25-007:IDEMPOTENCY-CONTRACT-FIX | SPR25-007 | OTHER | Commerce idempotency contract and implementation audit | PASS | Read-only audit of Prisma model, repository migration, PostgreSQL trigger/check constraints, migration history, and all application insert paths |
| EVIDENCE:SPR25-007:IDEMPOTENCY-CONTRACT-FIX | SPR25-007 | TEST | Initial-state regression coverage | PASS | Focused payment service Jest suites and PGlite migration/trigger contract suite |
| EVIDENCE:SPR25-007:IDEMPOTENCY-CONTRACT-FIX | SPR25-007 | BUILD | Backend schema, build, endpoint-security, and dependency review | PASS | prisma:validate; prisma:generate; build; security:audit-endpoints; npm audit --audit-level=high |
| EVIDENCE:SPR25-007:IDEMPOTENCY-CONTRACT-FIX | SPR25-007 | DEPLOY | Exact-revision CI, migration deployment, and process restart | PASS | GitHub Actions Backend CI #156 and Deploy Backend Production #114 |
| EVIDENCE:SPR25-007:IDEMPOTENCY-CONTRACT-FIX | SPR25-007 | PRODUCTION | Post-deploy provider-disabled boundary | PASS | Body-discarding HTTPS GET /health and malformed POST /api/v1/payments/webhooks/payos |
| EVIDENCE:SPR25-007:LOCAL-SECURITY-READINESS | SPR25-007 | TEST | Complete deterministic PayOS security and failure-recovery matrix | PASS | npm.cmd test -- --runInBand src/modules/payments src/config/env.validation.spec.ts src/config/production-commerce-config.spec.ts |
| EVIDENCE:SPR25-007:LOCAL-SECURITY-READINESS | SPR25-007 | TEST | Payment persistence, migration, and immutable-history matrix | PASS | node --experimental-vm-modules ./node_modules/jest/bin/jest.js --runInBand --config prisma/jest.config.cjs payment-request-migration.spec.ts payment-receiving-account-migration.spec.ts payment-reconciliation-migration.spec.ts commerce-fulfillment-migration.spec.ts commerce-persistence-contract.spec.ts commerce-persistence-migration.spec.ts |
| EVIDENCE:SPR25-007:LOCAL-SECURITY-READINESS | SPR25-007 | TEST | Learner checkout and administrator operations UI matrix | PASS | npm.cmd test -- --run src/features/payments/PaymentCheckout.test.tsx src/features/dashboard/AdminDashboard/AdminCommercePage.test.tsx src/services/commerce.service.test.ts src/services/admin-commerce.service.test.ts |
...
| EVIDENCE:SPR25-007:PRODUCTION-UAT-TARGET-READINESS | SPR25-007 | PRODUCTION | Reconfirm current provider boundary | BLOCKED | Body-discarding HTTPS checks for health and malformed PayOS webhook |
| EVIDENCE:SPR25-007:PROVIDER-ACTIVATION | SPR25-007 | MANUAL | Production Commerce activation preflight | PASS | npm run config:verify:production-commerce on /var/www/eduai/backend |
| EVIDENCE:SPR25-007:PROVIDER-ACTIVATION | SPR25-007 | MANUAL | Production process restart after activation | PASS | npm run process:restart:production on /var/www/eduai/backend |
| EVIDENCE:SPR25-007:PROVIDER-ACTIVATION | SPR25-007 | PRODUCTION | Active provider boundary and public readiness | PASS | Body-discarding HTTPS checks for health, return, cancel, and malformed PayOS webhook |
| EVIDENCE:SPR25-007:PROVIDER-ACTIVATION | SPR25-007 | PRODUCTION | Dedicated target and identity revalidation | PASS | Sanitized Playwright production GET-only target and reconciliation preflight |
...
| EVIDENCE:SPR25-007:PROVIDER-DISABLED-ROLLBACK | SPR25-007 | MANUAL | Production Commerce preflight after operator rollback | PASS | npm run config:verify:production-commerce on /var/www/eduai/backend |
| EVIDENCE:SPR25-007:PROVIDER-DISABLED-ROLLBACK | SPR25-007 | MANUAL | Production process restart after operator rollback | PASS | npm run process:restart:production on /var/www/eduai/backend |
| EVIDENCE:SPR25-007:PROVIDER-DISABLED-ROLLBACK | SPR25-007 | PRODUCTION | External provider-disabled readiness verification | PASS | Body-discarding HTTPS checks for health and malformed PayOS webhook |
| EVIDENCE:SPR25-007:PROVIDER-ROLLBACK-RECHECK | SPR25-007 | MANUAL | Verify provider-disabled production boundary after operator rollback | BLOCKED | Production health GET and body-discarding malformed PayOS webhook after PAYOS_ENVIRONMENT=disabled |
| EVIDENCE:SPR25-007:PROVIDER-ROLLBACK-RECHECK | SPR25-007 | OTHER | Compare local configuration without exposing secrets | BLOCKED | npm run config:verify:production-commerce |
| EVIDENCE:SPR25-007:UAT-TARGET-SETUP | SPR25-007 | PRODUCTION | Dedicated course target prepared for bounded live UAT | PASS | Sanitized Playwright production target setup through guarded instructor and administrator APIs |
| EVIDENCE:SPR25-007:UAT-TARGET-SETUP | SPR25-007 | PRODUCTION | Dedicated membership target prepared for bounded live UAT | PASS | Sanitized Playwright production target setup through guarded administrator and learner APIs |
